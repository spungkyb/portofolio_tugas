...
//buat kode otomatis
	$query_oto = mysql_query("select max(NomorByr) as maksi from bayarcicilan");
...
<center>
  ...
  <form name="input_bayarCicilan" method="post" action="proses.php">
    <table width="450" height="220" border="0"  class="bayarcicilan">
      ...
      <tr>
        ...
        <select name="kodeKredit" id="kodeKredit" onChange="getNamaCust()">
		  <option value=""></option>
			<?php
				$sqlKodeKredit = mysql_query("select KodeKredit from belikredit");
				while ($kodeKredit=mysql_fetch_array($sqlKodeKredit)){
	 			echo "<option value=$kodeKredit[KodeKredit]>$kodeKredit[KodeKredit]</option>";
				}
			?>
          </select>
         </td>
